...
<jsp:body>
	...
	<main role="main" style="background-color: #f5f5f5;padding: 1rem;">
		<div class="container wrapper" style="max-width: 900px;min-height: 800px;">	
			<h5 class="h5 mb-3 font-weight-normal">Gerenciar/Chamados > Mensagens</h5>
			<hr/>
			<div style="width:100%;">
				 <c:choose>
				 	<c:when test="${chamadoSolicitante.status eq 'Concluído'}">
				 		<div style="font-size: .8rem;background-color: #5bc0de  !important;padding: 10px;border-radius: .25rem;">
					 		<label><i class="fas fa-check"></i> Este chamado não permite novas mensagens!</label>
				 		</div>
				 		<br/>				 	
				 	</c:when>
				 	<c:when test="${chamadoSolicitante.status ne 'Concluído'}">
					 	 	<form:form id="mensagemCadForm" name="mensagemCadForm" commandName="mensagem">
								<div class="form-group row" style="margin-left: 5%;">
									...
									<div class="form-group col-4" style="margin-top: 50px;">
									...
									<div class="col-8" style="margin-top:-15px;">
										<form:select id="status" path="statusChamado" class="form-control form-control-sm">
										  	<form:option value="Em atendimento" label="Status - Selecione..."/>
									        <form:option value="Em atendimento" label="Em atendimento"/>					        			       
									        <form:option value="Aguardando retorno do solicitante" label="Aguardando retorno do solicitante"/>					        			       
									        <form:option value="Aguardando terceiros" label="Aguardando terceiros"/>
									    </form:select>	
									</div>
								</div>
...
$(function() {
 ...
 $('#btnAdicionarMensagem').click(function(e) {
    ...
    $.post({
  	 url : "${s:mvcUrl('COC#adicionarMensagem').build() }", 
