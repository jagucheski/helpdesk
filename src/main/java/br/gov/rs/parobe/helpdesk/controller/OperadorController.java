	...
	public UsuarioJsonResponse excluir(@ModelAttribute Usuario usuarioExcluir) {
		...
		try {
			...
			if(usuarioLog.getEmail().equals(usuarioExcluir.getEmail())) {
				errors.put("usuario",	"Você não pode excluir sua conta de operador");
				response.setValidated(false);
				response.setErrorMessages(errors);				
			}else if(operadorDAO.existMensagens(usuarioExcluir.getEmail())) {
				errors.put("mensagens",	"Ops...Este Operador não pode excluido, pois possui chamados vinculados.");
				response.setValidated(false);
				response.setErrorMessages(errors);						
			}else {
