...
public class CategoriaController {
	...
	public CategoriaJsonResponse excluirCategoria(@ModelAttribute Categoria categoriaExcluir) {
		...
		try {
			...
			if (categoria.getSubCategorias().size() > 0 || categoria.getChamados().size() > 0) {
				errors.put("subCategoriaChamadoo", "Ops... Esta Categoria não pode ser excluida, pois possui registros vinculados.");
				response.setValidated(false);
				response.setErrorMessages(errors);
			} else {
			...
			errors.put("errorCategoria", "Ops... Ocorreu um erro inesperado.  " + e.getMessage().toString());
			response.setValidated(false);
			response.setErrorMessages(errors);
		}
		return response;
	...
	/**
	 * Editar dados da categoria através do mapping /editarCategoria Permissão de
	 * acesso: SUP
	 **/
	@Transactional
