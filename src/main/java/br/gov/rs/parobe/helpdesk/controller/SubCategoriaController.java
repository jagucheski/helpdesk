...
public class SubCategoriaController {
	...
	public SubCategoriaJsonResponse excluirSubCategoria(@ModelAttribute SubCategoria subCategoriaExcluir) {
		...
		try {
			...
			if (subCategoria.getCategorias().size() > 0 || subCategoria.getChamados().size() > 0){
		        errors.put("registros",  "Ops... Esta Sub-Categoria não pode ser excluida, pois possui registros vinculados.");
	            response.setValidated(false);
		        response.setErrorMessages(errors);	
			}else {
		    ...
		    errors.put("subCategoria", "Ops... Ocorreu um erro inesperado.  " + e.getMessage().toString());
            response.setValidated(false);
	        response.setErrorMessages(errors);
		}
		return response;
